---
slug: openclaw-audit-allowlist-identity-hunter-demo
name: Allowlist Identity Audit — Hunter (OpenClaw chat-channel extensions, demo build)
...
3. **`group-policy.ts`** — when present, encapsulates group-route
   matching helpers (e.g. `find<Channel>GroupEntry`,
   `build<Channel>GroupCandidates`). Frequently the place where
...
5. **DM vs group, and the parallel groups block.** Channels with both
   ...
   resolution block frequently gate one and forget the other — adjacent
   blocks are commonly fixed independently. Audit each block
...
The published Slack writeup passes this gate cleanly because:
(i) the config is what the docs tell operators to write,
...
leaks out from behind the flag." `telegram` is patched but useful
as a fix reference.
