    ...
    patterns:
      - regex: (speakeasy|otplib|@otplib|pyotp|rotp|node-2fa|google-authenticator|twofactor|two-factor-auth)
        ...
        notIn:
          ...
          - '**/node_modules/**'
        label: TOTP/MFA library
      - regex: (verify|validate|check)[._]?([Tt]otp|[Oo]tp|[Mm]fa|[Tt]wo[_]?[Ff]actor|2fa)\b|(totp|authenticator|otplib)\s*\.\s*(verify|check|validate)
        in:
          - '**/*.{ts,tsx,js,jsx,mjs,cjs}'
          - '**/*.{py,rb,go,php,java,kt,cs}'
        notIn:
...
**4. No rate limit on OTP submission.** TOTP is 6 digits → 10⁶ space.
...
if (valid) {
  return res.json({ token: jwt.sign({ sub: user.id, amr: ["pwd"] }, SECRET) });
}
...
The most reliable test in your head: "if I never call /verify-2fa,
what protected endpoints can I hit with the response from /login?"
If the answer is "all of them", it's a bypass.
